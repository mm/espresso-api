...
class AuthService:
    ...
    def _split_bearer_token(cls, header_token: str) -> str:
        ...
        if (
            (len(split_header) == 2)
            and (split_header[0].lower() == "bearer")
            and (split_header[1])
        ):
    ...
    def associate_external_user(cls, uid: str) -> Union[int, None]:
        ...
        user = User.query.filter(
            or_(User.email == user_info["email"], User.external_uid == uid)
        ).first()
        if not user:
            user_id = User.create(
                name=user_info["name"], firebase_uid=uid, email=user_info["email"]
            )
        else:
    ...
    def check_link_access(cls, user_id: int, link: Link) -> Union[None, bool]:
        ...
        if user_id != link.user_id:
            raise AuthError(
                message="You are not authorized to access this item", status_code=403
            )
        return True
